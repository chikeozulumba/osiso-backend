...
import { countriesToAlphaThree } from '../data/countries';
import { COUNTRIES_AND_STATES } from '../data/countries-and-states';
...
for (const key in countries) {
  if (Object.prototype.hasOwnProperty.call(countries, key)) {
    const entity = countries[key];
    const slug = slugify(entity.name.toLowerCase(), '-');
    COUNTRIES.push({
      ...entity,
      alphaTwo: key,
      alphaThree: countriesToAlphaThree[key],
      slug,
      active: key === 'NG' ? 1 : 0,
      resourceUri:
        process.env.CLIENT_APP_URL + '/countries/' + slug + '-' + Date.now(),
    });
  }
}

export default class CountriesAndStatesSeeder implements Seeder {
  public async run(factory: Factory, connection: Connection): Promise<void> {
    for (let i = 0; i < COUNTRIES.length; i++) {
      let country = COUNTRIES[i];
      const countryWithStates = COUNTRIES_AND_STATES.find(
        (c) => c.code2 === country.alphaTwo,
      );

      if (
        Array.isArray(countryWithStates?.states) &&
        countryWithStates?.states.length > 0
      ) {
        const repository = await connection.getRepository(Country);
        country = await repository.save(repository.create(country));
        countryWithStates.states.forEach(async (state) => {
          const stateRepository = await connection.getRepository(State);
